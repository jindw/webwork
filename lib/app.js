...
'any|get|post|update:put|patch|del:delete'.split('|').map(function(methodName){
    ...
    App[methodName] = (callback,path) => defaultApp[methodName](callback,path);
...
function doFilter(app,req,resp,impl,config){
    ...
    function innerNext(req,resp,chainDepth){
        ...
        return impl instanceof Function ? impl.call(model,req,resp):Promise.resolve(impl);
    }
    return result.then(
            value=>doResolveViewModel(app,req,resp,value,model),
            error=>{
