...
class App{
    constructor(prefix){
        ...
        this.views = [];
        this.prefix = prefix || '';
        this.receiver = (req,resp)=>doAccept(app,req,resp)
    }
    start(port,callback){
        this.server = require('http').createServer(this.receiver);
...
function doAccept(app,req,resp){
    ...
    return doFilter(app,req,resp,()=>Promise.resolve(404),{})//405:,'No Matched Router'
}
...
function doResolveViewModel(app,req,resp,view,model){
    if(typeof view == 'number'){
        ...
        resp.writeHead(200,{"Content-Type":'text/json;charset=utf-8'})//default json stringify model
    }
    return stringifyPromiseMap(model).then(body=>{//json data output
        return resp.end(body)||body.length;
    });
}
function stringifyPromiseMap(model){
