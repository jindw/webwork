...
function doFilter(app,req,resp,impl,config){
    ...
    function innerNext(req,resp){
        while(startIndex<len){
            ...
            let result = filter.call(model,req,resp,innerNext,config);
            return waitResult(filter,result);
        }
        return impl instanceof Function ? waitResult(impl,impl.call(model,req,resp)):Promise.resolve(impl);
    }
    return innerNext(req,resp).then(function(value){
