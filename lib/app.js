...
function doAccept(app,req,resp,next){
    ...
    if(!prefix || path.startsWith(prefix)){
        ...
        let startIndex = 0;
        let prefixEnd = prefix && prefix.length || 0;
        let queryIndex = url.indexOf('?');
        let path = queryIndex>0? url.substring(prefixEnd,queryIndex): prefixEnd? url.substr(prefixEnd):url;
...
function doFilter(app,req,resp,impl,config){
    let initializer = setupRequest(req);
...
function toAsync(fn){
    return fn instanceof AsyncFunction? fn:fn&&doWrap(fn);
}
function doWrap(fn){
...
const setupRequest = toAsync(require('./input-parser'));
