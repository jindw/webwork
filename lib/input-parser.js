...
function parseBody(req){
    ...
    if(transferEncoding || !isNaN(contentLength)){//has body http://www.w3.org/Protocols/rfc2616/rfc2616-sec4.html#sec4.3
        ...
        let type = contentType.replace(/.*(\bjson)$|.*\/x\-www\-form\-(urlencoded)/,'$1$2');
        return read(req,MAX_BODY_LEN).then(
                buf=> type=='json'?JSON.parse(String(buf)):type=='urlencoded'?buildQuery(String(buf)):buf
            )
    }
}
        ...
        let events = {
            data(chunk){
                ...
                if(received>maxLength){
                    clean('bytes received exceeds the limit ')
                }
            },
            end(){clean()},
            close(){clean('closed')},
